...
export function resolveRegistrationEntries(data: IncomingRegistrationData): ResolvedRegistrationEntry[] {
    ...
    const selectedDay = slotOptions.find((slot) => slot.date === data.preferredDate)
    ...
    const age = getAgeFromDobYear(data.dateOfBirth)
    if (age === null) {
